...
namespace fadsdb{
    ...
    SymbolParser& SymbolParser::get_attr(string& key, string& val, bool& end)
    {
        if (error_){
            return *this;
        }
        ...
        do{
            ...
            if (get_token(key).get_sep(sep).error()){
                break;
            }
            ...
            if (get_token(val).get_sep(sep).error()){
                break;
            }
            ...
            SOCE_DEBUG << _S("get_attr key", key) << _S("val", val);
            return *this;
        }while(0);
    ...
    int SymbolParser::load(const std::string& file, std::string& schema)
    {
        ...
        if (!is){
            ...
            is.read(const_cast<char*>(schema.c_str()),length);
            is.close();
        }
